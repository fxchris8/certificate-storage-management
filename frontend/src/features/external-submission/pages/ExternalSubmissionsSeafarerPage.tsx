...
import { FileText, Eye, Search, ChevronLeft, ChevronRight, ArrowLeft } from 'lucide-react';

const LIMIT = 10;

export function ExternalSubmissionsSeafarerPage() {
  const { seafarercode: seafarerCode } = useParams<{ seafarercode: string }>();
  ...
  const seafarerName = submissions.length > 0 ? submissions[0].seafarerName : '';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-zinc-500">
        <button
          onClick={() => navigate('/dashboard/external-submissions')}
      ...
      <div className="flex items-start gap-4">
        <Button
          variant="outline"
          onClick={() => navigate('/dashboard/external-submissions')}
          className="h-10 gap-2 border-zinc-200 hover:bg-zinc-50 shrink-0"
        >
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="relative w-full sm:w-auto sm:min-w-[280px]">
          ...
          <Input
            placeholder="Search by certificate name or nomor..."
            value={searchInput}
              ...
              <TableBody>
                {submissions.map((submission) => (
                  <TableRow key={submission.id}>
                    <TableCell className="font-medium">{submission.certificateName}</TableCell>
