...
import { FileText, Anchor, LogOut } from "lucide-react"
import { Button } from "@/components/ui/button"
import { removeCrewToken } from "@/lib/cookies"
...
export function CrewSidebar({ collapsed }: CrewSidebarProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={cn(
            "flex h-10 w-10 items-center justify-center rounded-lg bg-teal-600 text-white shadow-md shadow-teal-600/20 transition-all",
            collapsed ? "mr-0" : "mr-3"
          )}
        >
        ...
        <div>
          ...
          <nav className="space-y-1">
            {links.map((link) => {
              ...
              // Exact match for /crew root, startsWith for others
              const isActive =
                link.href === "/crew"
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "bg-teal-50 text-teal-600 shadow-sm dark:bg-teal-950/20 dark:text-teal-400"
      ...
      <div ref={logoutRef} className="p-4 border-t border-zinc-100 dark:border-zinc-900 w-full relative">
        {showLogoutConfirm && (
          ...
          >
            ...
            {/* Arrow pointer */}
            <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-white border-b border-r border-zinc-200 rotate-45 dark:bg-zinc-950 dark:border-zinc-800" />
          </div>
        )}
