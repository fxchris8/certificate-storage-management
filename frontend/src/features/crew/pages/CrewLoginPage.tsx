...
import { useCrewLogin } from "../_hooks/useCrewLogin";
import { CrewLoginFormData, crewLoginSchema } from "../types/crew.types";
import { zodResolver } from "@hookform/resolvers/zod";
import { useForm } from "react-hook-form";
import { Anchor, ShieldAlert } from "lucide-react";

export function CrewLoginPage() {
  const { mutate: login, isPending: isLoading, error } = useCrewLogin();

  const form = useForm<CrewLoginFormData>({
  ...
  const onSubmit = (data: CrewLoginFormData) => {
    login(data);
  };
      ...
      <Card className="w-full max-w-md shadow-lg border-opacity-40 bg-white/80 backdrop-blur-sm dark:bg-slate-950/50">
        ...
        <CardContent>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              {error && (
                <div className="flex items-center gap-2 p-3 text-sm text-red-600 bg-red-50 dark:bg-red-950/20 dark:text-red-400 rounded-lg">
                  ...
                  <p>
                    Login gagal. Silakan periksa kembali seafarer code dan password Anda.
                  </p>
                </div>
              )}
          ...
          <Button
            ...
            asChild
          >
            <Link to="/crew/register">Register</Link>
          </Button>
        </CardContent>
