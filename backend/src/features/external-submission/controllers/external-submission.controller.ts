...
export class ExternalSubmissionController {
  ...
  getSubmissions = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const seafarerCode = req.query.seafarerCode as string | undefined;
  ...
  getGroupedSeafarers = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const page = req.query.page ? parseInt(req.query.page as string) : undefined;
      const limit = req.query.limit ? parseInt(req.query.limit as string) : undefined;
      const search = req.query.search as string | undefined;

      const result = await this.externalSubmissionService.getGroupedSeafarers(
        page,
        limit,
        search,
      );
